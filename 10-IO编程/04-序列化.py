...
import json
d2 = dict(name = 'Tom', age = 30, score = 98)
str2 = json.dumps(d2)
print(str2)
# dumps()方法返回一个str，内容就是标准的JSON。类似的，dumo()方法可以直接把JSON写入一个file-like Object.
# 要把JSON反序列化为Python对象，用loads()或者对应的load()方法，前者把JSON的字符串反序列化，后者从file-like
# Object中读取字符串并反序列化：
json_str = '{"name": "Tom", "age": 30, "score": 98}'
d3 = json.loads(json_str)
print(d3)
# 由于JSON标准规定JSON编码是UTF-8，所以我们总是能正确地在Python str与JSON的字符串之间转换


# JSON进阶
# Python的dict对象可以直接序列化为JSON的{}, 不过，很多时候，我们喜欢用class表示对象，比如定义Student类，
# 然后序列化：
import json
...
# 别急，我们仔细看看dumps()方法的参数列表，可以发现，除了第一个必须的obj参数外，dumps()方法还提供一大堆的可选参数
# 这些可选参数就是让我们定制JSON序列化，前面的代码之所以无法把Student类实例序列化JSON，是因为默认情况下，
...
# 这样，Student实例首先被StudentToDict()函数转换为dict，然后再被顺利序列化为JSON
json_str2 = json.dumps(s, default = StudentToDict)
print(json_str2)
...
json_student = json.loads(json_str2, object_hook = dict2student)
print(json_student)
